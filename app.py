...
# ---------------------------------------------------------------------------
# Modelo estricto para electrodomésticos (admite /, -, letras y números de 4 a 25 caracteres)
...
WEB_NOISE_KEYWORDS = [
    'recíbelo entre', 'entrega garantizada', 'programas de lavado', 'tipo de instalación',
    'añadir a la cesta', 'añadir al carrito', 'añadir al carro', 'política de cookies',
    'política de privacidad', 'todos los derechos reservados', 'gastos de envío',
    'opiniones de los clientes', 'ver valoraciones', 'filtrar por', 'ordenar por',
    'página 1 de', 'iniciar sesión', 'crear cuenta', 'atención al cliente',
    'derechos reservados', 'aceptar todas', 'cookies estrictamente', 'financiación sin intereses',
    'envío gratis', 'stock disponible', 'en stock', 'agotado', 'últimas unidades',
    'garantía de', 'vendido por', 'gestionado por', 'añadir a la lista', 'ver ficha técnica',
    'descargar manual', 'comparar producto'
]

SECTION_CUTOFF_KEYWORDS = [
    'productos relacionados', 'también te puede interesar', 'quien vio esto también vio',
    'comprados juntos habitualmente', 'accesorios recomendados', 'productos similares',
    'ofertas destacadas en', 'artículos patrocinados'
]

def preprocess_clipboard_text(text: str) -> str:
    ...
    # Normalizar comillas y guiones unicode
    cleaned = cleaned.replace('–', '-').replace('—', '-').replace('’', "'")
    
    # 3. Eliminar espacios múltiples consecutivos horizontales (manteniendo saltos de línea)
...
def format_excel_file(filepath: str):
    try:
        ...
        expensive_fill = PatternFill(start_color="FCE8E6", end_color="FCE8E6", fill_type="solid") # Soft red/coral
        expensive_font = Font(name="Segoe UI", size=10, bold=False, color="C5221F")
        ...
        for idx, header in enumerate(headers):
            if header and str(header).startswith("Precio "):
                col_letter = get_column_letter(idx + 1)
                h_str = str(header)
                if "Sin IVA" in h_str:
                    price_cols_by_cat['no_vat'].append((col_letter, idx + 1))
                elif "Con IVA" in h_str:
        ...
        for row_idx in range(2, num_rows + 1):
            ...
            for col_idx in range(1, len(headers) + 1):
                ...
                if header_val.startswith("Diferencia / Oportunidad"):
            ...
            # Highlight minimum price and maximum price cell
            for cat, cols in price_cols_by_cat.items():
                ...
                for col_letter, col_idx in cols:
                    ...
                    if val is not None:
                        try:
                            f_val = float(clean_price(str(val)) if not isinstance(val, (int, float)) else val)
                            if f_val > 0:
                                valid_count += 1
                                if f_val < min_val:
                                    min_val = f_val
                ...
                if min_cell is not None and min_val != float('inf') and valid_count > 1:
                    min_cell.fill = cheap_fill
                    min_cell.font = cheap_font
                    if max_cell is not None and max_cell != min_cell and max_val > min_val and cat == 'no_vat':
        ...
        for col in ws.columns:
            ...
            for cell in sample_cells:
                ...
                for line in lines:
                    if len(line) > max_len:
                        max_len = len(line)
            if header_name.startswith("Diferencia / Oportunidad"):
                ws.column_dimensions[col_letter].width = max(max_len + 4, 55)
            else:
...
def normalize_and_reorder_product_prices(item: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    for k in all_price_keys:
        val_str = str(item.get(k) or "").strip()
        num = clean_price(val_str)
        if num > 0:
            price_entries.append((k, val_str, num))
    ...
    for entry in price_entries:
        rounded = round(entry[2], 2)
        if rounded not in seen_nums:
...
TITLE_NOISE_WORDS = [
    r'¡?oferta\s*(?:top|flash|especial)?!?', r'¡?chollo!?', r'¡?descuento!?',
    r'¡?precio\s*especial!?', r'¡?promoci[oó]n!?', r'¡?comprar!?', r'¡?nuevo\s*(?:modelo)?!?',
    r'¡?top\s*ventas!?', r'¡?mejor\s*precio!?', r'¡?env[ií]o\s*gratis!?', r'¡?en\s*stock!?',
    r'¡?disponible!?', r'¡?env[ií]o\s*\d+\s*h!?', r'¡?venda\s*flash!?'
]
_RE_TITLE_NOISE = re.compile(r'\b(?:' + '|'.join(TITLE_NOISE_WORDS) + r')\b', re.IGNORECASE)

COLOR_PATTERNS = [
    (re.compile(r'\b(?:acero\s*inoxidable|acero\s*inox|inox|antihuellas)\b', re.IGNORECASE), 'Acero Inoxidable'),
    (re.compile(r'\b(?:cristal\s*negro)\b', re.IGNORECASE), 'Cristal Negro'),
    (re.compile(r'\b(?:cristal\s*blanco)\b', re.IGNORECASE), 'Cristal Blanco'),
    (re.compile(r'\b(?:blanco|blanca)\b', re.IGNORECASE), 'Blanco'),
    (re.compile(r'\b(?:negro|negra|black)\b', re.IGNORECASE), 'Negro'),
    (re.compile(r'\b(?:grafito|antracita|gris|silver|plata)\b', re.IGNORECASE), 'Grafito / Gris'),
    (re.compile(r'\b(?:integrable|encastrable|panelable)\b', re.IGNORECASE), 'Integrable'),
]

def clean_and_normalize_product_name(title: str, model: str = "") -> str:
    if not title:
        return ""
    cleaned = _RE_TITLE_NOISE.sub("", title)
    cleaned = re.sub(r'[!¡?¿*]', '', cleaned)
    ...
    formatted = []
    for w in words:
        if w.isupper() and len(w) <= 4 and any(c.isdigit() for c in w):
            formatted.append(w)
        elif w.isupper() and len(w) > 1 and not w.isalpha():
            formatted.append(w)
        else:
            formatted.append(w.capitalize())
    clean_title = " ".join(formatted)
    
    if model and model.upper() not in clean_title.upper():
        clean_title = f"{clean_title} ({model})"
    return clean_title

def extract_color_and_energy_class(text: str, category: str = ""):
    color = ""
    ...
    if not color and any(k in category.lower() for k in ["inducci", "vitro", "cristal gas"]):
        color = "Negro"
        
    energy = ""
    m = re.search(r'\b(?:clase\s+(?:energ[eé]tica\s+)?|eficiencia\s+(?:energ[eé]tica\s+)?|etiqueta\s+)?([A-Ga-g](?:\+{1,3})?)(?!\w)', text, re.IGNORECASE)
    if m:
        val = m.group(1).upper()
        if val in ['A+++', 'A++', 'A+', 'A', 'B', 'C', 'D', 'E', 'F', 'G']:
            energy = val
    if not energy:
        m2 = re.search(r'\bclase\s+([A-Ga-g](?:\+{1,3})?)(?!\w)', text, re.IGNORECASE)
        if m2:
...
def extract_products_adaptively(text: str, provider: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if isinstance(dict_data, dict):
            marcas = dict_data.get("marcas", {})
            if isinstance(marcas, dict):
                for k in marcas.keys():
                    if len(k.strip()) >= 2:
                        provider_keywords.add(k.strip().lower())
            elif isinstance(marcas, list):
                for m in marcas:
                    if isinstance(m, str) and len(m.strip()) >= 2:
                        provider_keywords.add(m.strip().lower())
    except Exception:
        pass
        
    provider_keywords.update({"balay", "bosch", "siemens", "samsung", "lg", "beko", "aeg", "whirlpool", "haier", "teka", "candy", "cecotec", "liebherr", "hisense", "electrolux", "indesit"})
...
def process_text(text: str, provider: Dict[str, Any], is_ocr: bool = False):
    ...
    try:
        ...
        if regex_pattern and regex_pattern.strip() and regex_pattern.strip() != ".*":
            ...
            try:
                ...
                add_log("warning", f"Error ejecutando regex de '{provider.get('name')}': {str(rx_err)}. Derivando a extractor adaptativo.")
                matches = []
                
            max_match_len = max(15000, len(text))
            matched_intervals = []
        ...
        # Si no hubo coincidencias exactas por regex, comprobar palabras clave o estructura antes de la extracción adaptativa
        ...
        for segment in unmatched_segments:
            ...
            if adaptive_matches:
                ...
                for item in adaptive_matches:
                    data = {}
                    all_p_matches = item.get("all_prices", [])
                    price_tuples = []
                    for p_str in all_p_matches:
                        v_num = clean_price(p_str)
                        if v_num > 0 and not any(abs(v_num - existing[1]) < 0.001 for existing in price_tuples):
                            price_tuples.append((p_str, v_num))
                    price_tuples.sort(key=lambda x: x[1])

                    p_lowest = price_tuples[0][0] if price_tuples else item.get("price", "No disponible")
                    ...
                    if not data:
                        data = item
                    normalize_and_reorder_product_prices(data)
...
async def save_provider(provider: ProviderModel):
    ...
    # Asegurar regex no nulo ni vacío
    clean_regex = (provider_dict.get("regex") or "").strip()
    if not clean_regex or clean_regex == "^...$":
        clean_regex = ".*"
    provider_dict["regex"] = clean_regex
    
    # Validar compilabilidad de la expresión regular
    try:
        re.compile(clean_regex)
    except Exception as rx_err:
        add_log("warning", f"El regex para '{provider.name}' tiene sintaxis especial o inválida: {str(rx_err)}")
        
    # Sincronizar fields a partir de las etiquetas definidas y de los grupos nombrados del regex
    extracted_fields = []
    if provider_dict.get("labels"):
        for l in provider_dict["labels"]:
            if isinstance(l, dict) and "name" in l and l["name"] not in extracted_fields:
                extracted_fields.append(l["name"])
                
    # Extraer grupos nombrados del regex si los tiene (ej: (?P<modelo>...) o (?<modelo>...))
    if clean_regex and clean_regex != ".*":
        group_names = re.findall(r'\(\?P<([a-zA-Z_][a-zA-Z0-9_]*)>', clean_regex) or re.findall(r'\(\?<([a-zA-Z_][a-zA-Z0-9_]*)>', clean_regex)
        for g in group_names:
            if g not in extracted_fields:
                extracted_fields.append(g)
                
    if extracted_fields:
        provider_dict["fields"] = extracted_fields
...
async def generate_regex_pattern(req: RegexGenerateRequest):
    ...
    def get_generic_pattern(label_name: str, label_val: str, is_last_on_line: bool, is_last_overall: bool) -> str:
        ...
        # Si es un modelo/SKU: es alfanumérico y tiene cierta estructura (admite guiones y barras)
    ...
    def get_transition_pattern(literal: str) -> str:
        ...
        words = literal.split()
        if not words:
            # Si contiene saltos de línea, reflejarlo
            if "\n" in literal:
                return r"\s*[\r\n]+\s*"
            return r"\s+"
        ...
        for w in words:
            ...
            if not w_clean:
                # El token completo está formado por signos de puntuación (ej: ',', '-', '/', ':')
                word_pats.append(re.escape(w))
                continue
                
            prefix = w[:w.find(w_clean)] if w_clean in w else ""
            suffix = w[w.find(w_clean)+len(w_clean):] if w_clean in w else ""
    ...
    for i, label in enumerate(sorted_labels):
        start = max(0, min(label.start, len(raw_text)))
        end = max(start, min(label.end, len(raw_text)))
        if start < last_idx:
            start = last_idx
            end = max(start, end)
        ...
        # Garantizar nombre de grupo único en Python regex
        group_name = name
        counter = 1
    ...
    try:
        ...
        if match:
            raw_extracted = match.groupdict()
            # Mapear de nombres únicos a nombres de etiquetas originales
            extracted = {}
            for lbl, gname in label_to_group_name:
                extracted[lbl.name] = raw_extracted.get(gname, "")
                
            # Validar que los valores extraídos coincidan aceptablemente con lo que se seleccionó
            mismatches = []
            for lbl, gname in label_to_group_name:
                name = lbl.name
                expected_val = raw_text[lbl.start:lbl.end].strip()
                actual_val = raw_extracted.get(gname, "").strip()
                ...
                if clean_exp != clean_act and expected_val != actual_val:
                    # Tolerar si la diferencia es solo una coma/guión inicial o final
                    trim_act = actual_val.strip(" ,-:/;|")
                    trim_exp = expected_val.strip(" ,-:/;|")
                    if trim_act != trim_exp:
            ...
            if mismatches:
                return {
                    ...
                    "message": f"La expresión regular coincide pero los valores extraídos difieren levemente: {'; '.join(mismatches)}"
                }
            else:
                return {
                    "status": "success",
...
async def test_regex_endpoint(req: RegexTestRequest):
    pattern = (req.regex or "").strip()
    text = (req.text or "").strip()
    if not pattern or pattern == "^...$":
        return {"status": "error", "message": "No se proporcionó un regex válido.", "matches": [], "count": 0}
    if not text:
        return {"status": "error", "message": "No se proporcionó texto de muestra.", "matches": [], "count": 0}
    ...
    try:
        compiled = re.compile(pat_clean)
    except Exception as e:
        return {"status": "error", "message": f"Error de sintaxis en el Regex: {str(e)}", "matches": [], "count": 0}
    ...
    try:
        for m in compiled.finditer(text):
            gdict = m.groupdict()
            if not gdict:
                gdict = {"coincidencia": m.group(0)}
            matches.append(gdict)
    except Exception as e:
        return {"status": "error", "message": f"Error durante la ejecución del Regex: {str(e)}", "matches": [], "count": 0}
    ...
    if matches:
        return {
            ...
            "matches": matches,
            "extracted": matches[0],
            "message": f"¡Éxito! El regex coincide con {len(matches)} elemento(s) en el texto de muestra."
        }
    else:
        return {
            "status": "warning",
            ...
            "matches": [],
            "extracted": {},
            "message": "El regex actual no coincide con el texto de muestra."
        }
...
def merge_extractions(req: MergeRequest):
    ...
    for filename in req.files:
        ...
        # Detectar columnas de precios (Sin IVA, Con IVA, PVP o General)
        ...
        for col in df.columns:
            ...
            is_no_vat = (
                col_norm in ['pr', 'coste', 'costo', 'neto', 'siniva', 'novat', 'precioneto', 'priceneto']
                or any(t in col_norm for t in ['siniva', 'novat', 'sin_iva', 'preciosiniva', 'pricesiniva', 'pricenovat', 'psiniva', 'sinivaeur'])
            )
            is_vat = (
                col_norm in ['pv', 'vat', 'coniva', 'precioconiva', 'pricevat']
                or any(t in col_norm for t in ['coniva', 'pricevat', 'withvat', 'precioconiva', 'pconiva'])
                or (col_norm.endswith('vat') and 'novat' not in col_norm and len(col_norm) <= 8)
            )
            is_pvp = (
                col_norm in ['pvp', 'pvpalto', 'pvpprecio', 'preciopvp', 'pricepvp']
                or 'pvp' in col_norm
                or 'precioventa' in col_norm
            )
            is_general = (
                col_norm in ['price', 'precio', 'importe', 'cifra', 'valor']
                or any(t in col_norm for t in ['precio', 'price', 'importe'])
            )
        ...
        if desc_src_col and desc_src_col not in selected_cols:
            selected_cols.append(desc_src_col)
    ...
    def resolve_cat(row):
        ...
        if k in cat_dict:
            return cat_dict[k]
        full_text = (str(row.get(key_col_title) or '') + ' ' + str(desc_dict.get(k, ''))).strip()
        return classify_product_category(full_text) if full_text else 'Otros'

    def resolve_brand(row):
        k = row['_merge_key_clean']
        if k in brand_dict:
            return brand_dict[k]
        p_str = (str(row.get(key_col_title) or '') + ' ' + str(desc_dict.get(k, '')) + ' ' + str(k)).lower()
    ...
    # Calcular diferencia / oportunidad estricta sobre el Precio Sin IVA
    def calculate_opportunity_sin_iva(row, cols):
        prices = {}
        for col in cols:
            val = row.get(col)
            if pd.notnull(val):
                p_clean = clean_price(str(val)) if not isinstance(val, (int, float)) else float(val)
                if p_clean > 0:
                    prices[col] = p_clean
        ...
        if len(prices) == 1:
            prov = list(prices.keys())[0].replace('Precio ', '').replace(' Sin IVA (€)', '').replace(' (€)', '').strip()
            p_val = list(prices.values())[0]
            return f"Solo en {prov} ({p_val:.2f} € Sin IVA)"
            
        sorted_prices = sorted(prices.items(), key=lambda x: x[1])
        cheapest_col, cheapest_price = sorted_prices[0]
        cheapest_name = cheapest_col.replace('Precio ', '').replace(' Sin IVA (€)', '').replace(' (€)', '').strip()
        
        # Comprobar si todos los precios son exactamente iguales
        if all(p == cheapest_price for _, p in sorted_prices):
            names = " = ".join([c.replace('Precio ', '').replace(' Sin IVA (€)', '').replace(' (€)', '').strip() for c, _ in sorted_prices])
            return f"Empate ({names} a {cheapest_price:.2f} € Sin IVA)"
            
        second_col, second_price = sorted_prices[1]
        ...
        if second_price == max_price:
            ...
            return f"🏆 {cheapest_name} más barato (-{diff_pct_cheaper:.1f}% | -{diff_money:.2f} € vs {second_name}) — {most_expensive_name} el más caro (+{max_diff:.2f} € | +{max_pct:.1f}%)"
        
    target_comp_cols = price_no_vat_cols if price_no_vat_cols else price_cols
    if target_comp_cols:
        merged_df['Diferencia / Oportunidad (Sin IVA)'] = merged_df.apply(
            lambda r: calculate_opportunity_sin_iva(r, target_comp_cols), axis=1
        )
    else:
        merged_df['Diferencia / Oportunidad (Sin IVA)'] = "Sin precios cargados"

    # Calcular la Gama por producto basándonos en los límites de precio de su categoría
    all_p_cols = price_no_vat_cols + price_cols + price_vat_cols + price_pvp_cols
