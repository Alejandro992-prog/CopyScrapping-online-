...
*   **Matriz de Stock y Cobertura (ERP PDF)**:
    *   **Funcionalidad**: Se creó una pestaña para que los compradores suban informes de stock en PDF de su ERP. El sistema analiza el texto del PDF e identifica los productos usando un diccionario semántico dinámico ([dictionary.json](file:///c:/Users/aleja/OneDrive/Escritorio/Programacion%20programas%20etc/ideas%20Nuevos%20Proyectos/CopyScrapping%20%28online%29/data/dictionary.json)). Cuenta con heurísticas adaptativas que autodetectan marcas y categorías nuevas no parametrizadas a partir del orden de palabras en la descripción.
    *   **Maquetación Visual**: Representa la cobertura física en una cuadrícula bidimensional cruzando **Marcas (Filas)** contra **Capacidades (Columnas)**. Cada celda de cruce contiene tres pastillas/píldoras interactivas correspondientes a las gamas **E (Económica)**, **M (Media)** y **P (Premium)**, que muestran el stock físico actual de esa gama (ej. `E: 12`) y están coloreadas mediante un código de colores semáforo interactivo. Estas pastillas actúan como filtros: al hacer clic sobre una gama, la tabla de detalles muestra únicamente los modelos correspondientes. Al pulsar sobre el fondo de la celda, se cargan todos los modelos combinados. También incluye un panel lateral de cuotas de inventario por marca.
...
*   **Procesamiento de EAN y Formato Tabular con Euro (€)**:
    *   **Parser Tabular**: Se incorporó soporte automático para líneas con el símbolo `€` en los informes de PDF del ERP. El motor detecta la estructura de columnas mapeando el coste (último número), stock (penúltimo número o cálculo de Total/Coste) y el código de barras/EAN (número de 8 o más dígitos escaneado de derecha a izquierda).
